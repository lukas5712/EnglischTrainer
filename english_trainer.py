...
class EnglishTrainerApp:
    ...
    def build_work(self):
        ...
        tk.Label(self.task_card, text="Mission Feed", font=("Segoe UI", 18, "bold"), fg=TEXT, bg=CARD).pack(anchor="w", padx=18, pady=(16, 10))
        ...
        tk.Label(self.result_card, text="Feedback Panel", font=("Segoe UI", 18, "bold"), fg=TEXT, bg=CARD_2).pack(anchor="w", padx=18, pady=(16, 10))
    ...
    def change_text_size(self, delta):
        ...
        self.output.config(font=("Consolas", max(13, self.text_size)))
        self.result_text.config(font=("Consolas", max(12, self.text_size - 1)))
    ...
    def require_live_generation(self):
        if not API_KEY:
            return "FEHLER: Bitte OPENAI_API_KEY setzen. Alle Aufgaben werden live mit ChatGPT erzeugt."
        return ""
    ...
    def get_local_sentence_task(self, direction, tense):
        tense = tense if tense in LOCAL_SENTENCES else "mixed"
        de, en = random.choice(LOCAL_SENTENCES.get(tense, LOCAL_SENTENCES["mixed"]))
        if direction == "de_to_en":
    ...
    def get_local_multiple_choice(self, direction, topic):
        ...
        while len(wrongs) < 3:
            wrongs.append(random.choice(random.choice(list(LOCAL_VOCAB.values()))))
        if direction == "de_to_en":
            question = correct_de
...
"""
    ...
    def start_multiple_choice_mode(self):
        ...
        self.entry_label.config(text="Choose one answer by clicking A, B, C or D")
        self.set_progress_text("")
        self.set_progress_bar(0, 100)
    ...
    def after_mc_loaded(self, result):
        ...
        self.entry_label.config(text="Choose one answer below by clicking A, B, C or D")
...
class LoginWindow:
    def __init__(self, root, on_success):
        ...
        tk.Label(body, text="OpenAI API Key", font=("Segoe UI", 11, "bold"), fg=TEXT, bg=CARD).pack(anchor="w")
    ...
    def login(self):
        ...
        if not email or not key or not api_key:
            self.msg.config(text="Please enter email, product key and OpenAI API key.")
            return
